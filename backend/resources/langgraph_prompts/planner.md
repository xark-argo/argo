...
3.  如果 `has_enough_context` 为 `false`，则继续进行步骤分解和计划制定：
    ...
    *   **动态列表处理策略 (Dynamic List Processing Strategy)**：
        ...
        *   **替换概括性步骤为细化子步骤（关键规则）**：当上一个步骤的结果产生了多个实体（如子链、人物、股票列表等）且需要分别处理时：
            ...
            - **单实体原则**：当需要分别处理实体时，每个子步骤只包含一个实体（不要分组）；例如：
              * “获取A的周线K线及最近一周股价”
              * “获取B的周线K线及最近一周股价”
              * ...
...
- 若某个步骤可以由“现有的研究发现（observations）”直接回答，请仅返回它与具体 observation 的映射；不要把 observation 的正文复制到输出中。
    ...
    "steps": [
      {
        ...
        // execution_res 不要在此输出
      }
      // ... 其他步骤
    ]
  },
  "fills": [
    { "step_title": "<需要被填充的步骤标题>", "observation_index": <整数索引> }
    // 仅返回映射，不要包含 observation 的正文
  ]
}
```
