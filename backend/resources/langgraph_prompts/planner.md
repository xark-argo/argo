...
**⚠️ 核心约束：步骤去重**
- 每个步骤必须是唯一的（实体+任务类型）组合
- 禁止生成语义重复的步骤，即使描述略有不同
- 输出前必须执行去重检查
...
**Replan约束：**
- 只生成新增的步骤，不重复生成已有的步骤
- 除被拆解的任务步骤外，已有步骤会自动保留，无需在steps中重新列出
- 被拆解的任务步骤必须在描述开头加上`<decomposed>`标记
- 被拆解步骤的`extra`字段标记为`sub_step_X`，其中X为唯一索引

**去重示例：**
- ❌ `"获取[实体]数据"` + `"收集[实体]内容"` = 重复
- ❌ `"分析[实体]指标"` + `"评估[实体]指标"` = 重复
- ✅ 每个实体每种任务类型只出现一次
...
3.  如果 `has_enough_context` 为 `false`，则继续进行步骤分解和计划制定：
    ...
    *   **动态列表处理策略 (Dynamic List Processing Strategy)**：
        ...
        *   **替换概括性步骤为细化子步骤（关键规则）**：当上一个步骤的结果产生了多个实体（如子链、人物、股票列表等）且需要分别处理时：
            ...
            - **正确示例**：基于前序步骤"筛选符合条件的[实体类型]"的结果，如果得到了具体实体列表，应该这样分解：
              * "分析[具体实体1]的[分析维度]"（使用前序步骤实际返回的具体名称）
              * "分析[具体实体2]的[分析维度]"（使用前序步骤实际返回的具体名称）
            - **错误示例**：使用占位符如"分析[实体类型][具体实体]"或"分析[实体类型][具体标识]"等，这是不允许的
            - 分解后的子步骤描述要体现具体的策略要求，使用前序步骤实际结果中的具体实体
            - **注意**：示例中的具体策略要求仅用于说明格式，实际任务中请根据用户的具体输入来生成相应的策略要求描述
...
**步骤类型与拆分规则：**
- **综合类步骤**：包含多个子任务或需要处理多个信息源的概括性步骤，可以被拆分为子步骤
- **具体化步骤**：已经明确具体任务和目标的步骤，**禁止再次拆分**
- **拆分判断标准**：只有当一个步骤描述中包含"多个"、"各种"、"全面"、"综合"等概括性词汇，且实际需要处理多个独立信息源时，才能考虑拆分
- **拆分标记要求**：被拆解的任务步骤必须在描述开头加上`<decomposed>`标记
...
- **去重要求**：输出前必须验证每个步骤的唯一性，确保与已有的计划没有重复语义的步骤。
- **Replan规则**：只生成被拆解的任务步骤和新增步骤，其他已有步骤会自动保留，无需重复生成。被拆解步骤必须在描述开头加上`<decomposed>`标记。
...
**去重提醒：**
- 确保每个步骤都是唯一的（实体+任务类型）组合
- 输出前验证没有重复语义的步骤
- **Replan注意**：只输出被拆解的任务步骤和新增步骤，其他已有步骤无需重复生成。被拆解步骤必须在描述开头加上`<decomposed>`标记。
