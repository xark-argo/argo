...
def create_react_agent(
    ...
    *,
    ...
    For more details on using `create_react_agent`, 
    ...
    Args:
        ...
        prompt: An optional prompt for the LLM. Can take a few different forms:
            ...
            - str: This is converted to a SystemMessage and \
                added to the beginning of the list of messages in state["messages"].
            - SystemMessage: this is added to the beginning of the list \
                of messages in state["messages"].
            - Callable: This function should take in full graph state and \
                the output is then passed to the language model.
            - Runnable: This runnable should take in full graph state and \
                the output is then passed to the language model.

        response_format: An optional schema for the final agent output.

            If provided, output will be formatted to match the given schema and \
                returned in the 'structured_response' state key.
            ...
            Can be passed in as:
                ...
                - a tuple (prompt, schema), where schema is one of the above.
                    The prompt will be used together with the model that is \
                        being used to generate the structured response.

            !!! Important
                `response_format` requires the model to support `.with_structured_output`

            !!! Note
                The graph will make a separate call to the LLM to generate the \
                    structured response after the agent loop is finished.
                This is not the only strategy to get structured responses, see more options \
                    in [this guide](https://langchain-ai.github.io/langgraph/how-tos/react-agent-structured-output/).

        pre_model_hook: An optional node to add before the `agent` node (i.e., the node that calls the LLM).
            Useful for managing long message histories (e.g., message trimming, summarization, etc.).
            Pre-model hook must be a callable or a runnable that takes in current graph \
            ...
            !!! Important
                At least one of `messages` or `llm_input_messages` MUST be provided and \
                    will be used as an input to the `agent` node.
                The rest of the keys will be added to the graph state.

            !!! Warning
                If you are returning `messages` in the pre-model hook, you should \
                    OVERWRITE the `messages` key by doing the following:
    ...
    The "agent" node calls the language model with the messages list \
    ...
    The "tools" node executes the tools (1 tool per `tool_call`) and \
        adds the responses to the messages list
    ...
    if isinstance(model, str):
        try:
            ...
            raise ImportError(
                ...
                '<provider>:<model>' string syntax for `model` parameter."
            )
    ...
    if _should_bind_tools(model, tool_classes) and len(tool_classes) > 0 or (len(llm_builtin_tools) > 0):
    ...
    def _are_more_steps_needed(state: StateSchema, response: BaseMessage) -> bool:
        has_tool_calls = isinstance(response, AIMessage) and len(response.tool_calls) > 0
